"""
Synthetic Data Modeling for Inverse Modeling of Elastic Thickness
Based on Braitenberg's work on the eastern Alps

This module provides functionality for creating synthetic topography and Moho data
and performing elastic thickness inversion using the convolution method.
"""
...
class MohoAnalysis:
    ...
    def read_surfer_grd(self, filepath):
        """
    ...
    def get_user_parameters(self, use_synthetic_data=True):
        ...
        use_synthetic_data : bool
            If True, prompts for domain/mountain/moho parameters.
            If False, only prompts for analysis parameters.
        """
        print("="*80)
        ...
        if use_synthetic_data:
            ...
            print("\n Using real data - domain parameters will be read from files")
    ...
    def moving_window_analysis(self, topography, moho_depth, params, X, Y):
        ...
        Takes moho_depth as input, not gravity
        """
        ...
        for shift_dist in shift_distances:
            ...
            # Optional: override physical parameters (useful for other planets)
            physics = params.get('physics', {})
            if physics:
                inverter.rho_load = physics.get('rho_load', inverter.rho_load)
                inverter.rho_m = physics.get('rho_m', inverter.rho_m)
                inverter.rho_infill = physics.get('rho_infill', inverter.rho_infill)
                inverter.g = physics.get('g', inverter.g)
    ...
    def plot_comprehensive_results(self, topography, gravity, moho_depth, results,
        ...
        # Calculate extent from actual coordinates
        x_min, x_max = np.min(X)/1000, np.max(X)/1000
        y_min, y_max = np.min(Y)/1000, np.max(Y)/1000
        
        # Topography
        im1 = axes[0,0].imshow(topography, extent=[x_min, x_max, y_min, y_max],
        ...
        # Add mountain markers (only if using synthetic data)
        if 'mountains' in params and params['mountains']['large']['height'] > 0:
            large = params['mountains']['large']
            small = params['mountains']['small']
        ...
        # Gravity
        im2 = axes[0,1].imshow(gravity, extent=[x_min, x_max, y_min, y_max],
                              cmap='viridis', origin='lower')
        ...
        # Moho depth
        im3 = axes[1,0].imshow(moho_depth/1000, extent=[x_min, x_max, y_min, y_max],
                              cmap='viridis', origin='lower')
        ...
        # Cross-sections
        mid_row = topography.shape[0] // 2
        x_km = X[mid_row, :] / 1000  # Use actual X coordinates
...
def main(topography_file=None, moho_file=None):
    ...
    topography_file : str, optional
        Path to Surfer .grd file containing topography data (meters)
    moho_file : str, optional
        Path to Surfer .grd file containing Moho depth data (meters)
    ...
    if use_synthetic_data:
        ...
        print("="*80)
        print("\nTo use your own data, provide paths to:")
        print("  - Topography .grd file (elevation in meters)")
        print("  - Moho depth .grd file (depth in meters)")
        print("\nExample:")
        print("  main(topography_file='path/to/topo.grd', moho_file='path/to/moho.grd')")
        print("\n" + "-"*80)
        
        # Get user parameters for synthetic data
        params = analyzer.get_user_parameters(use_synthetic_data=True)
        ...
        print(f"\nLoading topography from: {topography_file}")
        X, Y, topography, dx, dy, nx, ny = analyzer.read_surfer_grd(topography_file)
        
        # Load Moho depth data
        print(f"\nLoading Moho depth from: {moho_file}")
        X_moho, Y_moho, moho_depth, dx_moho, dy_moho, nx_moho, ny_moho = analyzer.read_surfer_grd(moho_file)
        
        # Check if grids match
        if nx != nx_moho or ny != ny_moho:
            print(f"\nWARNING: Grid dimensions don't match!")
            print(f"  Topography: {nx} x {ny}")
            print(f"  Moho: {nx_moho} x {ny_moho}")
            print("  Attempting to interpolate Moho to topography grid...")
            # Simple interpolation would go here if needed
            # For now, we'll assume they match or user will handle it
        
        # Get user parameters (for analysis settings only, skip domain/mountain/moho)
        params = analyzer.get_user_parameters(use_synthetic_data=False)
        
        # Set domain parameters from actual data
        params['grid_spacing'] = dx
        params['nx'] = nx
        params['ny'] = ny
        params['domain_size'] = np.max(X) - np.min(X)  # Use actual domain size
        
        # Add dummy mountains dict for plotting (not used in real data analysis)
        params['mountains'] = {
            'large': {'x': np.mean(X), 'y': np.mean(Y), 'width': 0, 'length': 0, 'height': 0},
            'small': {'x': np.mean(X), 'y': np.mean(Y), 'width': 0, 'length': 0, 'height': 0}
        }
        
        print(f"\nUsing data domain:")
        print(f"  Size: {(np.max(X) - np.min(X))/1000:.1f} x {(np.max(Y) - np.min(Y))/1000:.1f} km")
        print(f"  Grid: {nx} x {ny} points")
        print(f"  Spacing: {dx/1000:.2f} x {dy/1000:.2f} km")
